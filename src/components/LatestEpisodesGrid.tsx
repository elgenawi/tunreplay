  ...
  return (
    <section className="py-8 relative">
      <div className="container mx-auto px-4">
        ...
        {/* Mobile List */}
        <div className="sm:hidden space-y-4 space-x-4">
          {limitedEpisodes.map((episode, index) => (
            <Link key={`${episode.slug}-${index}`} href={`/episodes/${episode.slug}`}>
              <div className="justify-between mb-4 relative aspect-video rounded-lg overflow-hidden">
                <Image
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black via-black/50 to-transparent">
                  ...
                  <p className="text-sm text-white/80">الحلقة {episode.number}</p>
                </div>
              </div>
            </Link>
          ))}
