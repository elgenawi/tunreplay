...
function CompactSeriesCard({
  ...
  series: ScheduleItem["seies"][0]["series_id"];
}) {
  return (
    <Link
  ...
  const getWeekDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({
        date,
        dayName:
          i === 0 ? "اليوم" : daysAr[dayName as keyof typeof daysAr],
        englishName: dayName,
      });
    }
    return days;
    ...
    <main className="container mx-auto px-3 sm:px-4 py-16 sm:py-20">
      <div className="text-center mb-6 sm:mb-8">
        <div className="inline-block bg-white/10 rounded-lg px-4 sm:px-6 py-2 sm:py-3">
          <div className="text-white/60 mb-1 text-xs sm:text-sm">
            توقيت غرينتش +1
          </div>
          <div className="text-xl sm:text-2xl font-mono text-primary">
            {time}
          </div>
        </div>
      </div>
