...
export async function generateMetadata({
  ...
  if (!genre) {
    return { title: "Not Found", description: "The requested genre was not found." };
  }
  const title = `${genre.name} - TUNREPLAY`;
  const description = `مشاهدة احدث مسلسلات ${genre.name} مترجمة اون لاين وتحميل مباشر بجودة عالية HD على موقع TUNREPLAY`;
  return {
    title,
    description,
    keywords: [genre.name, "مسلسلات", "افلام", "مشاهدة", "تحميل", "اون لاين", "مترجم", "HD"],
...
export default async function Page({ params, searchParams }: PageProps) {
  ...
  const [genre, series, totalCount] = await Promise.all([
    getGenreBySlug(slug),
    getSeriesByGenreSlug(slug, currentPage),
    getSeriesCountByGenreSlug(slug),
  ]);
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([
            generateCollectionPageSchema({
              name: genre.name,
              description: `مشاهدة احدث مسلسلات ${genre.name} مترجمة اون لاين`,
